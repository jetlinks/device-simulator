...
@CommandLine.Command(name = "coap",
        ...
        description = {
                "Create CoAP Benchmark"
        },
        headerHeading = "%n", sortOptions = false)
class CoAPBenchMark extends AbstractBenchmarkCommand implements Runnable {
    ...
    protected Mono<? extends Connection> createConnection(ConnectCreateContext ctx) {
        ...
        return CoapClient.create(commandOptions);
    }

    static class CommandOptions extends CoapOptions {

        @Override
        @CommandLine.Option(names = {"--id"}, description = "ID", defaultValue = "coap-client-{index}", order = 1)
