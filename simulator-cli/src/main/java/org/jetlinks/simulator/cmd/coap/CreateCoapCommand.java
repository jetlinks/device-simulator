...
public class CreateCoapCommand extends AbstractCommand implements Runnable {
    ...
    public void run() {
        ...
        printf("create coap client %s", options.getId());
        try {
            ...
            main()
                    .getCommandLine()
                    .execute("coap", "attach", client.getId());
        } catch (Throwable err) {
    ...
    static class Options extends CoapOptions {
        ...
        @Override
        @CommandLine.Option(names = {"--id"}, description = "Client ID", defaultValue = "coap-client")
